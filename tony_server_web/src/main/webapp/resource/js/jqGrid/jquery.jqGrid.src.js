...
//				'nn':function(queryObj,op) {return op === "OR" ? queryObj.orNot().isNull : queryObj.andNot().isNull;}
		...
		setColWidth();
		$(eg).css("width","auto").append("<div class='ui-jqgrid-resize-mark' id='rs_m"+ts.p.id+"'>&#160;</div>");
		$(gv).css("width","auto");
		...
		if(ts.p.pager){
			...
			$(ts.p.pager).css({width: "auto"}).addClass('ui-state-default ui-jqgrid-pager ui-corner-bottom').appendTo(eg);
		...
		$(grid.bDiv)
			...
			.css({ height: ts.p.height+(isNaN(ts.p.height)?"":"px"), width: "auto"})
			.scroll(grid.scrollGrid);
		$("table:first",grid.bDiv).css({width:(ts.p.tblwidth+1) +"px"});
		if( !$.support.tbody ) { //IE
...
$.jgrid.extend({
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			$($t.grid.bDiv).css("width",(nwidth+1)+"px");
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		if ( $.fn.jqm && p.jqModal === true) {
			if(p.left ===0 && p.top===0 && p.overlay) {
				...
				//调整弹出框位置 huangbiao modify at 20160521 start
				//p.left = pos[0] + 4;
				//p.top = pos[1] + 4;
				p.left = screen.width/3;
				p.top = screen.height/5;
				//调整弹出框位置 huangbiao modify at 20160521 end
			}
			coord.top = p.top+"px";
			coord.left = p.left;
		...
		if (p.width === 0 || !p.width) {p.width = 400;}
		if(p.height === 0 || !p.height) {p.height =300;}
		if(!p.zIndex) {
		...
		$(mw).css($.extend({
			...
			zIndex:p.zIndex,
			//modify by sky for 下拉树 at 2016-09-01 start
			//overflow: 'hidden'
			overflow: 'inherit'
			//modify by sky for 下拉树 at 2016-09-01 end
		},coord))
	...
	createEl : function(eltype,options,vl,autowidth, ajaxso) {
		...
		switch (eltype)
		{
			...
			case "file" :
				...
				break;
			case "custom" :
				//elem = document.createElement("span"); delete  by sky
				try {
					if($.isFunction(options.custom_element)) {
						var celm = options.custom_element.call($t,vl,options);
						if(celm) {
							//modify by sky ------------start
							//celm = $(celm).addClass("customelement").attr({id:options.id,name:options.name,value:vl});
							//$(elem).empty().append(celm);
							celm = $(celm).attr({id:options.id,name:options.name,value:vl});
							elem=celm;
							//modify by sky ------------end
						} else {
							throw "e2";
			...
			case "channelTree":$
				try {
					var url = "/" + ump.mvcContextPath + "/selectChannelTreeInfo";
					if (!!options.menuCode) {
						url = url + "?menuCode=" + options.menuCode;
					}
					var celm = '<input name="tree_input" type="text" class="input-elm">'
						+ '<div name="tree_Div"style="z-index: 99999;display: none;position:absolute;overflow:scroll;  width: 300px!important;height:300px!important;  padding: 5px;background: #fff;color: #fff;border: 1px solid #cccccc">'
						+ '<div align="right"><a href="##" onclick="closeTree()" style="color:red">关闭&nbsp;</a></div>'
						+ '<div name="tree_content"></div></div>';
					celm = $(celm).attr({id: options.id, name: options.name, value: vl});
					elem = celm;
					$(elem[0]).bind("click", function(){
						channelTree = new dTree('channelTree', 'resource/imgmenu/', 'no', 'no');
						$.ajax({
							type: "post",
							url:url,
							async	:true,
					...
					$(elem[1]).bind("mouseleave focus blur", function(){
						closeTree()
					});
		}catch	(e)	{
			if (e === "e1") {
				$.jgrid.info_dialog($.jgrid.errors.errcap, "url can not be null", $.jgrid.edit.bClose);
			}
		}
		break;
		//add by sky ------------end
	...
	return this.each( function() {
		...
		*/
			...
			inputAddRule.bind('click',function() {
				...
				// but show only serchable and serchhidden = true fields
					...
					if ((ignoreHiding && searchable) || (searchable && !hidden)) {
						...
						//modify by sky for 查询条件弹出的时候不出现相同的字段 start
						//原来的代码-------------start
						//cm = that.p.columns[i];
						//break;
						//原来的代码------------end
						...
						var isExist = false;
						cm = that.p.columns[i];
						for(var s = 0; s < group.rules.length; s++) {
							if(group.rules[s].field == cm.name) {
								isExist = true;
								break;
							}
						}
						...
						//修改后的代码-----------end
						//modify by sky for 查询条件弹出的时候不出现相同的字段 end
					}
				}
				//add by sky for 如果所有查询条件都已经加载完毕，则点击查询框的 【+】的时候不加载重复的查询项 -----start
				if(!cm){
					return false;
				}
				//add by sky for 如果所有查询条件都已经加载完毕，则点击查询框的 【+】的时候不加载重复的查询项 -----end
				var opr;
				if( cm.searchoptions.sopt ) {opr = cm.searchoptions.sopt;}
				else if(that.p.sopt) { opr= that.p.sopt; }
				else if  ( $.inArray(cm.searchtype, that.p.strarr) !== -1 ) {opr = that.p.stropts;}
				else {opr = that.p.numopts;}
				  var ruleArray = group.rules;
				for(i = 0; i < ruleArray.length; i ++ ){
					ruleArray[i].data = decodeURIComponent(ruleArray[i].data);
				}
				group.rules.push({
					field: cm.name,
					op: opr[0],
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			//update by huangbiao for font satrt
			var ruleDataTd = $("<td class='data h6'></td>");
			//update by huangbiao for font end
			...
			//update by sky at 2016-09-19--------------------------------------------start
			//$(ruleDataInput)
			//.addClass("input-elm")
			//.bind('change', function() {
			...
			if(cm.inputtype === 'custom'){
				$(ruleDataInput[0])
					.addClass("input-elm")
					.bind('change', function() {
						rule.data = cm.searchoptions.custom_value.call($t, $(this).children(".customelement:first"),'get');
						that.onchange();
					});
			}else{
				$(ruleDataInput)
			...
			//update by sky at 2016-09-19--------------------------------------------end
			...
			if(this.p.ruleButtons === true) {
			var ruleDeleteInput = $("<input type='button' title='Delete rule' class='delete-rule ui-del'/>");
...
$.extend($.fn.jqFilter,{
	...
	filterData : function()
	{
		...
		var dataArray = s.rules;
		if (!dataArray) {
			s.rules = [];
		}else{
			for (var i = 0; i < dataArray.length; i++) {
				if (dataArray[i].data.indexOf("%") == -1) {
					dataArray[i].data = encodeURI(dataArray[i].data);
				}
			}
		}
		return s;
	...
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				// modify by zhangfeng  将此按键设置成文件下载功能
//				if(!p.showQuery) {   
				if(p.showDownload) {  
					bQ ="<a id='"+fid+"_query' class='fm-button ui-state-default ui-corner-all fm-button-icon-left ui-search btn btn-sm btn-info'><span class='ui-icon ui-icon-comment'></span>文件下载</a>";
				}
				if(!p.columns.length) {
				...
				bt = "<table class='EditTable' style='border:0px none;margin-top:5px' id='"+fid+"_2'><tbody><tr><td colspan='2'><hr class='ui-widget-content' style='margin:1px'/></td></tr><tr><td class='EditButton' style='text-align:"+align+"'>"+bC+bQ+tmpl+"</td><td class='EditButton' "+butleft+">"+bS+"</td></tr></tbody></table>";
				fid = $.jgrid.jqID( fid);
				$("#"+fid).jqFilter({
					columns : columns,
				...
				if(bQ) {
					...
					$("#"+fid+"_query").bind('click', function(){
						...
						//add by huangbiao for paraMustInput check
						var flag = paramMustCheck(filters, $t, fl);
						if(!flag){
							return flag;
						}
						if(p.errorcheck) {
						...
						$.extend($t.p.postData,sdata);  
						// 文件下载参数封装  
						var filedata = "fileDownfilters=true&";
						$.each($t.p.postData,function(index,val) {
						  filedata += (index+"="+val+"&");
						});
						//window.open($t.p.url+"?"+filedata);  // $t.p.url
						filedata += ("fileDownFlag=true");
						//var queryStr = $t.p.url.getQueryString();
						if( $t.p.url.indexOf("?") != -1){
							window.open($t.p.url+"&"+filedata);  // $t.p.url
						}else{
							window.open($t.p.url+"?"+filedata);  // $t.p.url
						}
						
//						delete sdata.fileDownFlag;
//						delete $t.p.postData.fileDownFlag;
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			function createData(rowid,obj,tb,maxcols){
				...
				//modify by huangbiao for 新增框字体 start
				tdtmpl = "<td class='CaptionTD'>&#160;</td><td class='DataTD h6'></td>", tmpl="", i; //*2
				//modify by huangbiao for 新增框字体 end
				for (i =1; i<=maxcols;i++) {
					tmpl += tdtmpl;
				}
				if(rowid !== '_empty') {
					...
					if (ret[0]) {
						if (rp_ge[$t.p.id].useDataProxy) {
							...
							if(ajaxOptions.url === "clientArray") {
								 ...
								 * 增加 在 ajax 调用 的初始参数
								 */
								if(rp_ge[$t.p.id].beforeAjaxOptions){
									if($.isFunction(rp_ge[$t.p.id].beforeAjaxOptions)){
										ajaxOptions = rp_ge[$t.p.id].beforeAjaxOptions(ajaxOptions);
									}
								}
								$.ajax(ajaxOptions); 
							}
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				if (o.refresh) {
					...
					.click(function(){
						//add by sky for delete the downLoad params
//						delete $t.p.postData.fileDownFlag;
						if (!$(this).hasClass('ui-state-disabled')) {
...
addSubGridCell :function (pos,iRow) {
	...
	return "<td role=\"gridcell\" aria-describedby=\""+sid+"_subgrid\" class=\"ui-sgcollapsed sgcollapsed\" "+prp+"><a style='cursor:pointer;'><span class=' "+ic+"'></span></a></td>";
},
			...
			if($(ts.rows[i]).hasClass('jqgrow')) {
				...
				$(ts.rows[i].cells[pos]).bind('click', function() {
					...
					if($(this).hasClass("sgcollapsed")) {
						...
						$(this).html("<a style='cursor:pointer;'><span class=' "+ts.p.subGridOptions.minusicon+"'></span></a>").removeClass("sgcollapsed").addClass("sgexpanded");
						...
						$(this).html("<a style='cursor:pointer;'><span class=' "+ts.p.subGridOptions.plusicon+"'></span></a>").removeClass("sgexpanded").addClass("sgcollapsed");
...
window.onresize=function(){
    ...
    for(var i = 0; i < theTabs.length; i++){
        var contentId = $(theTabs[i]).attr("onclick", "skyFortest(this)");
    }
};

function skyFortest(e) {
	var divs = $($(e).attr("href")).find("div");
//		alert($(e).attr("href") + "_" + $($(e).attr("href")).width());
	for (var h = 0; h < divs.length; h++) {
		if ($(divs[h]).width() == 0 || $(divs[h]).width() == 1) {
			$(divs[h]).width("auto");
		}
	}
}
...
//必填check
function paramMustCheck(filters, $t, fl){
//add by huangbiao for must param check -------start
	var filtersRuleArray = filters.rules;
	var columnsArray = $("#"+$t.p.id).jqGrid('getGridParam','colModel');
	var errorCols = "";
	for (var h = 0; h < columnsArray.length; h++) {
		var flg = false;
		var columnModel = columnsArray[h];
		var rules = columnModel.searchrules;
		if (!!rules && rules.required == true) {
			for (var i = 0; i < filtersRuleArray.length; i++) {
				var fieldName = filtersRuleArray[i].field;
				var datax = filtersRuleArray[i].data;
				if (columnModel.name == fieldName && datax) {
					flg = true;
					break;
				}
			}
			if (!flg) {
				var errorCol = columnModel.label;
				errorCols = errorCol + "、" + errorCols;
			}
		}
	}
	if(errorCols.length > 0){
		fl[0].p.errmsg = "【" + errorCols.substr(0, errorCols.lastIndexOf('、')) + "】：字段必需";
		fl[0].showError();
		return false;
	}
	return true;
	//add by huangbiao for must param check -------------end
}
